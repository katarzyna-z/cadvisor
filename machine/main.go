...
func GetNVMMemoryCapcity() {
	...
	fmt.Println(err)
	fmt.Println("The total DCPMM capacity in bytes memory mode", caps.memory_capacity)
	fmt.Println("The total DCPMM capacity in GB for memory mode.", float64(caps.memory_capacity)/1024.0/1024.0/1024.0)
}

func main() {
	// fmt.Println("test")
	// err, val := GetNVMAvgPowerBudget()
	// fmt.Println(err, val)
	GetNVMMemoryCapcity()
}
